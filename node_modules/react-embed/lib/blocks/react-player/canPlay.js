...
var canPlayFile = function (url) { return MATCH_URL_FILE.test(url); };
var MATCH_URL_FACEBOOK = /facebook\.com\/([^/?].+\/)?video(s|\.php)[/?].*$/;
var canPlayFacebook = function (url) { return MATCH_URL_FACEBOOK.test(url); };
var MATCH_VIDEO_URL_TWITCH = /(?:www\.|go\.)?twitch\.tv\/videos\/(\d+)($|\?)/;
var MATCH_CHANNEL_URL_TWITCH = /(?:www\.|go\.)?twitch\.tv\/([a-z0-9_]+)($|\?)/;
var canPlayTwitch = function (url) { return MATCH_VIDEO_URL_TWITCH.test(url) || MATCH_CHANNEL_URL_TWITCH.test(url); };
var MATCH_URL_DAILY_MOTION = /^(?:(?:https?):)?(?:\/\/)?(?:www\.)?(?:(?:dailymotion\.com(?:\/embed)?\/video)|dai\.ly)\/([a-zA-Z0-9]+)(?:_[\w_-]+)?$/;
var canPlayDailyMotion = function (url) { return MATCH_URL_DAILY_MOTION.test(url); };
...
var canPlayMixcloud = function (url) { return MATCH_URL_MIXCLOUD.test(url); };
var MATCH_URL_STREAMABLE = /streamable\.com\/([a-z0-9]+)$/;
var canPlayStreamable = function (url) { return MATCH_URL_STREAMABLE.test(url); };
var canPlay = function (url) {
    return canPlayFacebook(url) ||
        canPlayTwitch(url) ||
